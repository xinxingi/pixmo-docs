...
from ..prompts.math_prompts import GENERATE_MATH_TOPICS_PROMPT, NUM_TOPICS

from ordered_set import OrderedSet
from datasets.fingerprint import Hasher
from datadreamer.steps import DataSource, SuperStep, Prompt, zipped

class GenerateMathTopics(SuperStep):
    CONFIG_HASH = Hasher.hash(
        [PERSONAS, GENERATE_MATH_TOPICS_PROMPT, NUM_TOPICS]
    )
    ...
    def run(self):
        ...
        prompts_dataset = triplets_dataset.map(
            lambda row: {
                "prompt": GENERATE_MATH_TOPICS_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateMathTopics.CONFIG_HASH)
