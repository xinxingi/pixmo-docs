...
from ..prompts.math_prompts import GENERATE_MATH_CODE_LATEX_PROMPT
...
class GenerateMath(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_MATH_CODE_LATEX_PROMPT])
    ...
    def run(self):
        ...
        prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_MATH_CODE_LATEX_PROMPT.format(
                    question=json.loads(row["data"])["question"],
                )
            },
            lazy=False,
    ...
    @property
    def version(self):
        return hash(GenerateMath.CONFIG_HASH)
