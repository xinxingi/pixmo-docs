...
from ..prompts.math_prompts import GENERATE_MATH_DATA_PROMPT
from ..utils.utils import extract_math, is_math_valid

class GenerateMathData(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_MATH_DATA_PROMPT])
    ...
    def run(self):
        ...
        prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_MATH_DATA_PROMPT.format(
                    topic=row["topic"],
        ...
        filtered = combined.filter(
            lambda row: is_math_valid(row["data"]),
            lazy=False,
    ...
    @property
    def version(self):
        return hash(GenerateMathData.CONFIG_HASH)
