...
from ..prompts.math_prompts import GENERATE_MATH_QA_PROMPT

class GenerateMathQA(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_MATH_QA_PROMPT])
    ...
    def run(self):
        ...
        qa_prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_MATH_QA_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateMathQA.CONFIG_HASH)
