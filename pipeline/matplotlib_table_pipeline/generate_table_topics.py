...
from ..prompts.table_prompts import GENERATE_TABLE_TOPICS_PROMPT, NUM_TOPICS

from ordered_set import OrderedSet
from datasets.fingerprint import Hasher
from datadreamer.steps import DataSource, SuperStep, Prompt, zipped

class GenerateTableTopics(SuperStep):
    CONFIG_HASH = Hasher.hash(
        [PERSONAS, GENERATE_TABLE_TOPICS_PROMPT, NUM_TOPICS]
    )
    ...
    def run(self):
        ...
        prompts_dataset = triplets_dataset.map(
            lambda row: {
                "prompt": GENERATE_TABLE_TOPICS_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateTableTopics.CONFIG_HASH)
