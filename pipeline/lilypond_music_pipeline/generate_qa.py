...
from ..prompts.misc_prompts import GENERATE_MUSIC_QA_PROMPT

class GenerateMusicQA(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_MUSIC_QA_PROMPT])
    ...
    def run(self):
        ...
        qa_prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_MUSIC_QA_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateMusicQA.CONFIG_HASH)
