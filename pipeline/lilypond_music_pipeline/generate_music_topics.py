...
from ..prompts.misc_prompts import GENERATE_MUSIC_TOPICS_PROMPT, NUM_TOPICS

from ordered_set import OrderedSet
from datasets.fingerprint import Hasher
from datadreamer.steps import DataSource, SuperStep, Prompt, zipped

class GenerateMusicTopics(SuperStep):
    CONFIG_HASH = Hasher.hash(
        [PERSONAS, GENERATE_MUSIC_TOPICS_PROMPT, NUM_TOPICS]
    )
    ...
    def run(self):
        ...
        prompts_dataset = triplets_dataset.map(
            lambda row: {
                "prompt": GENERATE_MUSIC_TOPICS_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateMusicTopics.CONFIG_HASH)
