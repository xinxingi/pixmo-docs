...
class GenerateDiagram(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_DIAGRAM_CODE_GRAPHVIZ_PROMPT])
    ...
    def run(self):
        ...
        prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_DIAGRAM_CODE_GRAPHVIZ_PROMPT.format(
                    topic=row["topic"],
        ...
        def execute_code_and_generate_image(row, timeout=20):
            ...
            try:
                with warnings.catch_warnings():
                    ...
                    image = generate_diagram(json.loads(row["data"])) # noqa: F821
                    globals().pop("generate_diagram", None)
