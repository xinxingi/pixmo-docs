...
from ..prompts.document_prompts import GENERATE_DOCUMENT_QA_PROMPT

class GenerateDocumentQA(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_DOCUMENT_QA_PROMPT])
    ...
    def run(self):
        ...
        qa_prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_DOCUMENT_QA_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateDocumentQA.CONFIG_HASH)
