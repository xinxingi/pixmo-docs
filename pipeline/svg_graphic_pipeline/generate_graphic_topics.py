...
from ..prompts.graphic_prompts import GENERATE_GRAPHIC_TOPICS_PROMPT, NUM_TOPICS

from ordered_set import OrderedSet
from datasets.fingerprint import Hasher
from datadreamer.steps import DataSource, SuperStep, Prompt, zipped

class GenerateGraphicTopics(SuperStep):
    CONFIG_HASH = Hasher.hash(
        [PERSONAS, GENERATE_GRAPHIC_TOPICS_PROMPT, NUM_TOPICS]
    )
    ...
    def run(self):
        ...
        prompts_dataset = triplets_dataset.map(
            lambda row: {
                "prompt": GENERATE_GRAPHIC_TOPICS_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateGraphicTopics.CONFIG_HASH)
