...
from ..prompts.graphic_prompts import GENERATE_GRAPHIC_DATA_SVG_PROMPT
from ..utils.utils import extract_math_svg, is_math_graphic_valid

class GenerateGraphicData(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_GRAPHIC_DATA_SVG_PROMPT])
    ...
    def run(self):
        ...
        prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_GRAPHIC_DATA_SVG_PROMPT.format(
                    topic=row["topic"],
