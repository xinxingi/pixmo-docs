...
from ..prompts.misc_prompts import GENERATE_CHEMICAL_TOPICS_PROMPT, NUM_TOPICS

from ordered_set import OrderedSet
from datasets.fingerprint import Hasher
from datadreamer.steps import DataSource, SuperStep, Prompt, zipped

class GenerateChemicalTopics(SuperStep):
    CONFIG_HASH = Hasher.hash(
        [PERSONAS, GENERATE_CHEMICAL_TOPICS_PROMPT, NUM_TOPICS]
    )
    ...
    def run(self):
        ...
        prompts_dataset = triplets_dataset.map(
            lambda row: {
                "prompt": GENERATE_CHEMICAL_TOPICS_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateChemicalTopics.CONFIG_HASH)
