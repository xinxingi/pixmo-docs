...
from ..prompts.misc_prompts import GENERATE_CHEMICAL_DATA_PROMPT
from ..utils.utils import extract_SMILES, is_SMILE_valid

class GenerateChemicalData(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_CHEMICAL_DATA_PROMPT])
    ...
    def run(self):
        ...
        prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_CHEMICAL_DATA_PROMPT.format(
                    topic=row["topic"],
        ...
        # Remove any invalid SMILE objects
        filtered = combined.filter(
            lambda row: is_SMILE_valid(row["data"]),
            lazy=False,
            name="Remove invalid SMILE objects",
        )
    ...
    @property
    def version(self):
        return hash(GenerateChemicalData.CONFIG_HASH)
