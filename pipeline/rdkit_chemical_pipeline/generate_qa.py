...
from ..prompts.misc_prompts import GENERATE_CHEMICAL_QA_PROMPT

class GenerateChemicalQA(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_CHEMICAL_QA_PROMPT])
    ...
    def run(self):
        ...
        qa_prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_CHEMICAL_QA_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateChemicalQA.CONFIG_HASH)
