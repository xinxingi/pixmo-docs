...
from ..prompts.misc_prompts import GENERATE_CIRCUIT_QA_PROMPT

class GenerateCircuitQA(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_CIRCUIT_QA_PROMPT])
    ...
    def run(self):
        ...
        qa_prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_CIRCUIT_QA_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateCircuitQA.CONFIG_HASH)
