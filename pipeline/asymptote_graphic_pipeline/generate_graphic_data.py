...
from ..prompts.graphic_prompts import GENERATE_GRAPHIC_DATA_ASYMPTOTE_PROMPT
from ..utils.utils import extract_math_asymptote, is_math_graphic_valid

class GenerateGraphicData(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_GRAPHIC_DATA_ASYMPTOTE_PROMPT])
    ...
    def run(self):
        ...
        prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_GRAPHIC_DATA_ASYMPTOTE_PROMPT.format(
                    topic=row["topic"],
        ...
        generated_data = Prompt(
            ...
            args={
                ...
                "post_process": extract_math_asymptote,
                "temperature": 1.0,
                "top_p": 1.0,
            },
            outputs={
        ...
        filtered = combined.filter(
            lambda row: is_math_graphic_valid(row["data"]),
            lazy=False,
        ...
        filtered = filtered.map(
            lambda row: {"data": json.dumps(row["data"])},
            lazy=False,
    ...
    @property
    def version(self):
        return hash(GenerateGraphicData.CONFIG_HASH)