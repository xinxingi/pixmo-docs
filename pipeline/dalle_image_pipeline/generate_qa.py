...
from ..prompts.image_prompts import GENERATE_IMAGE_QA_PROMPT

class GenerateImageQA(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_IMAGE_QA_PROMPT])
    ...
    def run(self):
        ...
        qa_prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_IMAGE_QA_PROMPT.format(
                    topic=row["topic"], description=row["data"], code=row["code"], persona=json.loads(row["metadata"])["persona"], image_type=json.loads(row["metadata"])["image_type"]
                )
            },
    ...
    @property
    def version(self):
        return hash(GenerateImageQA.CONFIG_HASH)
