...
from ..prompts.image_prompts import GENERATE_IMAGE_TOPICS_PROMPT, NUM_TOPICS

from ordered_set import OrderedSet
from datasets.fingerprint import Hasher
from datadreamer.steps import DataSource, SuperStep, Prompt, zipped

class GenerateImageTopics(SuperStep):
    CONFIG_HASH = Hasher.hash(
        [PERSONAS, GENERATE_IMAGE_TOPICS_PROMPT, NUM_TOPICS]
    )
    ...
    def setup(self):
        ...
        self.register_arg("image_types", required=True, help="The image types to use.")
    ...
    def run(self):
        ...
        UNUSED_PERSONAS = list(OrderedSet(PERSONAS) - OrderedSet(USED_PERSONAS))
                    ...
                    "metadata": json.dumps(
                        {
                            ...
                            "persona": persona,
                            "image_type": random.choice(self.args["image_types"]),
                        }
                    )
                }
            )
        ...
        prompts_dataset = triplets_dataset.map(
            lambda row: {
                "prompt": GENERATE_IMAGE_TOPICS_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateImageTopics.CONFIG_HASH)
