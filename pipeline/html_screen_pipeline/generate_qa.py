...
from ..prompts.screen_prompts import GENERATE_SCREEN_QA_PROMPT

class GenerateScreenQA(SuperStep):
    CONFIG_HASH = Hasher.hash([GENERATE_SCREEN_QA_PROMPT])
    ...
    def run(self):
        ...
        qa_prompts_dataset = combined_inputs.map(
            lambda row: {
                "prompt": GENERATE_SCREEN_QA_PROMPT.format(
    ...
    @property
    def version(self):
        return hash(GenerateScreenQA.CONFIG_HASH)
